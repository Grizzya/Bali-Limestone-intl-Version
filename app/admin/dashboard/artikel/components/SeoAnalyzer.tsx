...
export default function SeoAnalyzer({ judul, konten }: SeoAnalyzerProps) {
  ...
  useEffect(() => {
    ...
    if (!keyword.trim()) {
      setScore(0);
      setChecklist([{ text: 'Enter a focus keyword to start SEO analysis', passed: false }]);
      return;
    }
    ...
    checks.push({
      text: kwInTitle ? 'Focus keyword found in title.' : 'Focus keyword NOT found in title.',
      passed: kwInTitle
    });
    ...
    checks.push({
      text: kwInEarlyContent ? 'Focus keyword appears in the opening paragraph.' : 'Focus keyword missing from the opening paragraph.',
    ...
    // 4. Cek Kepadatan Kata Kunci (Keyword Density)
    const matches = ktn.match(new RegExp(kw, 'g'));
        ...
        <ul className="space-y-2.5 text-xs text-gray-600">
          {checklist.map((item, idx) => (
            <li key={idx} className="flex items-start gap-2.5 leading-relaxed">
              ...
              <span className={item.passed ? 'text-gray-700' : 'text-gray-400 italic'}>
                {item.text}
              </span>
            </li>
          ))}
